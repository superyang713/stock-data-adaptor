...
from ic.base import ICBase
from ic.utils import no_pandas
...
class Stock(ICBase):
    ...
    Other attributes, please refer to _IEXBase class
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.symbols = list(map(lambda x: x.upper(), self.symbols))
        self.endpoints = []
        super().__init__(
            token,
            retry=retry,
    ...
    def get_intraday_prices(self, **kwargs):
        ...
        return (
            self._get_endpoint("intraday-prices", format=format, params=kwargs)
        )
    ...
    def get_price(self):
        ...
        def format(out):
            return pd.DataFrame.from_dict(
                out, orient="index", columns=["price"]
            )
        ...
        def format(out):
            ...
            for symbol in out:
                if out[symbol]:
                    ...
                    results[symbol] = pd.DataFrame()
            return (
                results[self.symbols[0]].T
                if self.single_symbol else results
            )
            ...
            for symbol in out:
                if out[symbol]:
                    results[symbol] = pd.DataFrame.from_dict(
                        {
                            d["reportDate"]: d
                            for d in out[symbol]["financials"]
                        },
                        orient="index",
                    )
                else:
